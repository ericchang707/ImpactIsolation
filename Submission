...
def minimax(player, game, time_left, depth, my_turn=True):
    ...
    """
    if depth == 0 or len(game.get_active_moves()) == 0 or time_left() < 0.1:
        return game.get_player_position(player), CustomPlayer.utility(player, game, my_turn)
    best_move = None
    val = -float("inf") if my_turn else float("inf")

    for move in game.get_active_moves():
        next_game, forecast, winner = game.forecast_move(move)
        _, score = minimax(player, next_game, time_left, depth-1, not my_turn)
...
def alphabeta(player, game, time_left, depth, alpha=float("-inf"), beta=float("inf"), my_turn=True):
    ...
    Returns:
        (tuple, int): best_move, val
    """
    if depth == 0 or len(game.get_active_moves()) == 0 or time_left() < 100:
        return game.get_player_position(player), CustomPlayer.utility(player, game, my_turn)
    best_move = None
    val = -float("inf") if my_turn else float("inf")
    ...
    for move in game.get_active_moves():
        next_game, forecast, winner = game.forecast_move(move)
        _, score = alphabeta(player, next_game, time_left, depth-1, alpha, beta, not my_turn)
